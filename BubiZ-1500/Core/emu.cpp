...
// Windows VK constants needed by emu.cpp
#ifndef VK_LSHIFT
...
#elif defined(USE_AUTO_KEY_CAPS_LOCK)
...
#endif
	// Copy VM screen data (screen[][] -> vm_screen_buffer)
	vm->draw_screen();
...
#if defined(USE_CART) || defined(USE_BINARY_FILE)
		   ...
		   check_file_extension(file_path, _T(".d77")) || check_file_extension(file_path, _T(".1dd"))) {
			...
			if(fio->Fopen(file_path, FILEIO_READ_BINARY)) {
				try {
					...
					uint32_t file_size = (uint32_t)fio->Ftell(), file_offset = 0;
...
// ----------------------------------------------------------------------------
// debugger stubs (for macOS - not implemented)
// ----------------------------------------------------------------------------
