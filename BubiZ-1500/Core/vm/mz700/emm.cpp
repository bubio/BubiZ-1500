...
void EMM::initialize()
{
	...
	if(fio->Fopen(create_rom_path(_T("EMM.ROM")), FILEIO_READ_BINARY) ||
	   fio->Fopen(create_rom_path(_T("EMM.BIN")), FILEIO_READ_BINARY)) {
...
void EMM::release()
{
	...
	if(modified) {
		FILEIO* fio = new FILEIO();
		if(fio->Fopen(create_rom_path(_T("EMM.BIN")), FILEIO_WRITE_BINARY)) {
