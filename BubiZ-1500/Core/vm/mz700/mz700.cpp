...
#if defined(_MZ800)
	psg->initialize_sound(rate, CPU_CLOCKS, 4000);
#elif defined(_MZ1500)
	psg_l->initialize_sound(rate, CPU_CLOCKS, 4000);
	psg_r->initialize_sound(rate, CPU_CLOCKS, 4000);
#endif
}
...
#elif defined(_MZ1500)
	if(ch-- == 0) {
		psg_l->set_volume(0, decibel_l, -32);   // Left PSG: R=mute
	} else if(ch-- == 0) {
		psg_r->set_volume(0, -32, decibel_r);   // Right PSG: L=mute
	} else
#endif
	if(ch-- == 0) {
