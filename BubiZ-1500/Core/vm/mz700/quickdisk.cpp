...
void QUICKDISK::open_disk(const _TCHAR* path)
{
	...
	if(fio->Fopen(path, FILEIO_READ_BINARY)) {
		...
		// check extension and detect format
		bool is_mzt_format = check_file_extension(file_path, _T(".mzt")) || check_file_extension(file_path, _T(".q20"));
		if(!is_mzt_format && check_file_extension(file_path, _T(".qdf"))) {
			// some .qdf files contain MZT data instead of raw QD data
			uint8_t detect[MZT_HEADER_SIZE];
			fio->Fread(detect, sizeof(detect), 1);
			fio->Fseek(0, FILEIO_SEEK_SET);
			if(detect[0] >= 0x01 && detect[0] <= 0x05) {
				// first byte is valid MZT file attribute, check filename area
				bool has_filename = true;
				for(int i = 1; i < 13; i++) {
					if(detect[i] == 0x00) break;
					if(detect[i] != 0x0d && (detect[i] < 0x20 || detect[i] > 0x7e)) {
						has_filename = false;
						break;
					}
				}
