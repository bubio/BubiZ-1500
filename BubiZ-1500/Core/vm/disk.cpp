...
void DISK::open(const _TCHAR* file_path, int bank)
{
	...
	if(fio->Fopen(file_path, FILEIO_READ_BINARY)) {
		...
		file_size.d = (uint32_t)fio->FileLength();
			...
			if(check_file_extension(dest_path, _T(".d88")) || check_file_extension(dest_path, _T(".d8e")) ||
			   check_file_extension(dest_path, _T(".d77")) || check_file_extension(dest_path, _T(".1dd"))) {
				if(fio->Fopen(dest_path, FILEIO_READ_BINARY)) {
					...
					uint32_t total_size = (uint32_t)fio->Ftell(), offset = 0;
...
bool DISK::cpdread_to_d88(FILEIO *fio)
{
	...
	int image_size = (int)fio->FileLength();
...
// from NFD r0�`���t�@�C���\���d�l 2001/01/22 LED
typedef struct {
...
// from NFD r1�`���t�@�C���\���d�l 2001/09/14 LED
typedef struct NFD_FILE_HEAD1 {
    ...
    DWORD dwTrackHead(int trk)                  /* �g���b�NID�ʒu           */
    {
