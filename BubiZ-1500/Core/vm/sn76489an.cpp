...
void SN76489AN::mix(int32_t* buffer, int cnt)
{
	...
	for(int i = 0; i < cnt; i++) {
		...
		for(int j = 0; j < 4; j++) {
			...
			ch[j].count -= diff;
			if(ch[j].count < 0) {
				int period_x256 = ch[j].period << 8;
				ch[j].count += period_x256;
				if(j == 3) {
				...
				// Prevent unbounded negative accumulation when diff > period
				if(ch[j].count < 0 && period_x256 > 0) {
					int extra = (-ch[j].count + period_x256 - 1) / period_x256;
					ch[j].count += extra * period_x256;
					if(j == 3) {
			...
			int32_t sample = (prev_signal != ch[j].signal && prev_count >= 0 && prev_count < diff) ? (ch[j].volume * (2 * prev_count - diff)) / diff : ch[j].volume;
