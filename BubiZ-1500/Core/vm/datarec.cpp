...
void DATAREC::initialize()
{
	...
	memset(rec_file_path, 0, sizeof(rec_file_path));
...
int DATAREC::load_wav_image(int offset)
{
	...
	if(samples > 0) {
		...
		if(!config.wave_shaper[drive_num]) {
			...
			int half_threshold = 0, hi_count = 0, lo_count = 0;
...
void DATAREC::save_wav_image()
{
	...
	uint32_t length = (uint32_t)rec_fio->Ftell();
